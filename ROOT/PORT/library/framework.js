...
const scriptElement = document.querySelector('script#JIXIEJIDIGUAN');
let Events = '';
if (scriptElement) {
    const srcValue = scriptElement.getAttribute('src');
    const hrefMatch = srcValue.match(/[?&]href=([^&]+)/);
    Events = hrefMatch ? decodeURIComponent(hrefMatch[1]) : '';
}

const resources = [
  { type: 'css', url: Events + '/ROOT/PORT/library/mdui.css' },
  { type: 'js', url: Events + '/ROOT/PORT/library/mdui.global.js' },
  { type: 'css', url: Events + '/ROOT/PORT/library/normalize.min.css' }
];
